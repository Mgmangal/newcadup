...
class AdtController extends Controller
{
    public function index(){
        return view('theme-one.adt.index');
    }
    ...
    {
        return view('theme-one.adt.report');
    }
    ...
    public function reportList(Request $request)
    {
		...
		foreach ($result as $key => $value) {
            ...
            $staff_list = '<a class="btn btn-sm btn-info" href="javascript:void(0);" onclick="printData(`'.route('user.adt.staff.download',$latest_id->id).'`);" >Download Employees List</a> &nbsp';
        	$staff_test_list = '<a class="btn btn-sm btn-warning" href="javascript:void(0);" onclick="printData(`'.route('user.adt.test.download',$latest_id->id).'`);">Download Test Report</a>';
    ...
    {
        return view('theme-one.adt.generate');
    }
    ...
    public function generateReport(Request $request)
    {
        ...
        $dataInsert->status = '1';
        $dataInsert->save();
        return redirect()->route('user.adt.report.generate')->with('success', 'Report Generated Successfully');
    }
    ...
    public function reportAllList(Request $request)
    {
        ...
        $data = array();
		foreach ($result as $key => $value) {
            $print = '<a class="btn btn-sm btn-danger" target="_blank" href="'.route('user.adt.staff.download',$value->generate_report_id).'" >Print</a> &nbsp';
            $view = '<a class="btn btn-sm btn-info" target="_blank" href="'.route('user.adt.test.download',$value->generate_report_id).'" >View</a> &nbsp';
            $sub_array = array();
            $sub_array[] = ++$key;
            $sub_array[] = date('d-m-Y',strtotime($value->created_at));
            $sub_array[] = date('d-m-Y',strtotime($value->created_for_date));
    ...
    public function downloadTestList($id)
    {
        ...
        return view('theme-one.adt.test_list',$data);
    }
    ...
    public function downloadStaffList($id,$flag='')
    {
        ...
        $data['date'] = $ids->created_for_date; 
        $data['flag'] = $flag; 
        return view('theme-one.adt.staff_list',$data);
    }
}