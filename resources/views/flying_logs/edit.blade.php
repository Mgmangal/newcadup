<x-app-layout>
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            <h3 class="card-title">Flying Details Edit</h3>
            <a href="{{url()->previous()}}" class="btn btn-primary btn-sm p-2">Back</a>
        </div>
        <div class="card-body">
            <form action="{{route('app.flying-details.update',$data->id)}}" method="POST" enctype="multipart/form-data" id="manageForm">
                @csrf
                @method('PUT')
                <div class="row m-3">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="aircraft_id" id="aircraft_id" class="form-control">
                                ...
                                <option {{old('aircraft_id',$data->aircraft_id)==$aircraft->id?'selected':''}} value="{{$aircraft->id}}">{{$aircraft->call_sign}}-{{$aircraft->manufacturer}}-{{$aircraft->type_model}}</option>
                                @endforeach
                            </select>
                            @error('aircraft_id')
                            <span class="text-danger">{{$message}}</span>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="passenger[]" id="passenger" class="form-control selctPassenger" multiple>
                                <option value="">Select Passenger</option>
                                @foreach($passengers as $passenger)
                                <option {{in_array($passenger->more_data,$data->passenger)?'selected':''}} value="{{$passenger->more_data}}">{{$passenger->more_data}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="row m-3">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="pilot1_id" id="pilot1_id" class="form-control is_valid pilot1_id" onchange="changeHandlerPilotOne(this);">
                                <option value="">Select</option>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="pilot2_id" id="pilot2_id" class="form-control is_valid pilot2_id" onchange="changeHandlerPilotTwo(this);">
                                <option value="">Select</option>
                                @foreach($pilots as $pilot)
                                <option {{old('pilot2_id',$data->pilot2_id)==$pilot->id?'selected':''}} value="{{$pilot->id}}">{{$pilot->name}}</option>
                                @endforeach
                            </select>
                            @error('pilot2_id')
                            <span class="text-danger">{{$message}}</span>
                ...
                <div class="row m-3">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control auto_complete_input" name="comment" value="{{old('comment',$data->comment)}}" placeholder="Please Enter Comment">
                        </div>
                    </div>
                </div>
                <div class="row m-3">
