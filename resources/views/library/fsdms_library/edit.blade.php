<x-app-layout>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{route('app.library.fsdms_update',$fsdms->id)}}" method="POST" enctype="multipart/form-data" id="manageForm">
                @csrf
                @method('PUT')
                <div class="row m-3">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="title" name="title" placeholder="Please Enter Library Name" value="{{old('title',$fsdms->title)}}">
                            @error('title')
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select name="parent_id" id="parent_id" class="form-control">
                                ...
                                <option {{$fsdms->parent_id == $library->id ? 'selected':''}} value="{{$library->id}}">{{$library->title}}</option>
                                @endforeach
                            </select>
                            @error('parent_id')
                            <span class="text-danger">{{$message}}</span>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="description" id="description" cols="30" rows="5" placeholder="Please Enter Description">{{old('description',$fsdms->description)}}</textarea>
                            @error('description')
                            <span class="text-danger">{{$message}}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            @enderror
                            @if(!empty($fsdms->file))
                            <a target="blank" href="{{asset('uploads/library/'.$fsdms->file)}}" class="btn btn-sm btn-success">View File</a>
                            @endif
                        </div>
                    </div>
