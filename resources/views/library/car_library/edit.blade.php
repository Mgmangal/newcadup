<x-app-layout>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{route('app.library.car_update',$car->id)}}" method="POST" enctype="multipart/form-data" id="manageForm">
                @csrf
                @method('PUT')
                <div class="row m-3">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="title" name="title" placeholder="Please Enter Library Name" value="{{old('title',$car->title)}}">
                            @error('title')
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select name="parent_id" id="parent_id" class="form-control">
                                ...
                                <option {{$car->parent_id == $library->id ? 'selected':''}} value="{{$library->id}}">{{$library->title}}</option>
                                @endforeach
                            </select>
                            @error('parent_id')
                            <span class="text-danger">{{$message}}</span>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="description" id="description" cols="30" rows="5" placeholder="Please Enter Description">{{old('description',$car->description)}}</textarea>
                            @error('description')
                            <span class="text-danger">{{$message}}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            @enderror
                            @if(!empty($car->file))
                            <a target="blank" href="{{asset('uploads/library/'.$car->file)}}" class="btn btn-sm btn-success">View File</a>
                            @endif
                        </div>
                    </div>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            window.location.href = "{{ route('app.library.car') }}";
                        } else {
