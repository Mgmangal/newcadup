    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalLabel">Manage ATA Category</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{route('user.ata.category_store')}}" method="POST" id="manageForm" class="">
                @csrf
...
<script>
    function dataList() {
        ...
        $('#datatableDefault').DataTable({
            ...
            ajax: {
                url: "{{route('user.ata.category_list')}}",
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}"
                },
    ...
    {
        $.ajax({
            url: "{{route('user.ata.category_status')}}",
            type: 'post',
