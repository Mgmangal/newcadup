        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalLabel">Manage Section</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{route('app.settings.sections.store')}}" method="POST" id="manageForm" class="">
                    @csrf
        ...
        <script>
            function dataList() {
                ...
                $('#datatableDefault').DataTable({
                    ...
                    ajax: {
                        url: "{{route('app.settings.sections.list')}}",
                        type: 'POST',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
            ...
            function editRole(url) {
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            $('#manageForm').find('[name=department_id]').val(response.data.parent_id);
