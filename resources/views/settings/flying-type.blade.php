<x-app-layout>
    <x-slot name="breadcrumb">
        <ul class="breadcrumb">
            ...
            <li class="breadcrumb-item active">FLYING TYPE</li>
        </ul>
    </x-slot>
    <x-slot name="css">
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            <h3 class="card-title">Flying Type List </h3>
            <div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalLabel">Manage Flying Type</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{route('app.settings.flyingtype.store')}}" method="POST" id="manageForm" class="">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
                        <input type="hidden" name="edit_id" id="edit_id" />
                            <label class="form-label">Flying Type</label>
                            <input type="text" class="form-control" name="name" placeholder="Enter Flying Type" />
                        </div>
                    </div>
        ...
        <script>
            function dataList() {
                ...
                $('#datatableDefault').DataTable({
                    ...
                    ajax: {
                        url: "{{route('app.settings.flyingtype.list')}}",
                        type: 'POST',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
