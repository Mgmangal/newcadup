<x-app-layout>
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            <h3 class="card-title">None Flying Details Edit</h3>
            <a href="{{url()->previous()}}" class="btn btn-primary btn-sm p-2">Back</a>
        </div>
        <div class="card-body">
            <form action="{{route('app.none-flying-details.update',$data->id)}}" method="POST" enctype="multipart/form-data" id="manageForm">
                @csrf
                @method('PUT')
                ...
                <div class="row m-3">
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="user_id" class="form-label">Pilot<span class="text-danger">*</span></label>
                            <select name="user_id" id="user_id" class="form-control is_valid user_id" onchange="changeHandlerPilotOne(this);">
                                <option value="">Select</option>
                                @foreach($pilots as $pilot)
                                <option {{old('user_id',$data->user_id)==$pilot->id?'selected':''}} value="{{$pilot->id}}">{{$pilot->name}}</option>
                                @endforeach
                            </select>
                            @error('user_id')
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="from_dates" class="form-label">Departure Time</label>
                            <div class="input-group">
                                <label class="input-group-text" for="from_dates">
                                    <i class="fa fa-calendar"></i>
                                </label>
                                <input type="text" class="form-control datestime from_dates is_valid" onchange="calculateBlockTime(this);" id="from_dates" name="from_dates" value="{{is_get_date_time_format($data->from_dates)}}" onchange="validateDate();">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="to_dates" class="form-label">Arrival Time</label>
                            <div class="input-group">
                                <label class="input-group-text" for="to_dates">
                                    <i class="fa fa-calendar"></i>
                                </label>
                                <input type="text" class="form-control datestime to_dates is_valid" onchange="calculateBlockTime(this);" id="to_dates" name="to_dates" value="{{is_get_date_time_format($data->to_dates)}}" onchange="validateDate();" />
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="pilots" class="form-label">Block Time</label>
                            <input type="text" class="form-control block_time" id="block_time" name="block_time" placeholder="Please Enter Block Time" value="{{is_time_defrence($data->from_dates,$data->to_dates)}}" readonly>
                        </div>
                    </div>
                </div>
                ...
                <div class="row m-3 " >
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control reason" id="reason" name="reason" placeholder="Please Enter Reason" value="{{$data->reason}}">
                        </div>
                    </div>
                </div>
                ...
                <div class="row m-3 text-center">
                    <div class="col-md-12 ">
                        ...
                        <a href="javascript:void(0);" id="totalBlockTime" class="btn btn-warning" style="margin: 10px;">Total Block Time : {{is_time_defrence($data->from_dates,$data->to_dates)}}</a>
                    </div>
                </div>
            </form>
        </div>
